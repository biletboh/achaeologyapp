...
urlpatterns = [
        ...
        url(
            r'^(?P<pk>[0-9]+)/edit/$', views.SiteEdit.as_view(),
            name='editsite'),
        url(
            r'^(?P<pk>[0-9]+)/delete/$', views.SiteDelete.as_view(),
            name='deletesite'),
        ...
        url(
            ...
            {'template_name': 'archapp/login.html'}, name='login'),
        url(
            r'^accounts/logout/$', auth_views.logout, 
            {'template_name': 'archapp/logout.html'}, name='logout'),

        # user account
        url(
            r'user/(?P<slug>[\w.@+-]+)/profile/$',
            views.UserProfileAndCreateProject.as_view(), name='userprofile'),
        url(
            r'user/(?P<slug>[\w.@+-]+)/update/$',
            views.UserUpdate.as_view(), name = 'user-update'),
        url(
            r'user/(?P<slug>[\w.@+-]+)/delete/$', 
